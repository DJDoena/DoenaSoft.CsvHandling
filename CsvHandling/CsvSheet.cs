...
/// </summary>
/// <remarks>
/// <para>When working with sheets that have header rows, each header name must be unique for identification purposes.</para>
/// <para>Rows with inconsistent column counts are automatically normalized: shorter rows are padded with empty cells, 
/// and longer rows automatically extend the sheet's column count.</para>
/// </remarks>
public sealed class CsvSheet : ICsvSheet
{
    ...
    public void AddHeaderRow(IEnumerable<string> row)
    {
        ...
        for (var columnIndex = 0; columnIndex < row.Count(); columnIndex++)
        {
            ...
            columnName = MakeCleanCell(columnName);

            if (!seenNames.Add(columnName))
            {
                throw new ArgumentException($"Duplicate column name: '{columnName}'. Column names must be unique.");
            }
